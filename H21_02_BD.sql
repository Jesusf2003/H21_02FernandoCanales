...
CREATE TABLE BLOQUE(
    CODBLOQ     INT IDENTITY(1,1) PRIMARY KEY NOT NULL,
    NOMBLOQ     VARCHAR(100) NOT NULL,
    FECREGBLOQ  DATETIME NOT NULL DEFAULT GETDATE(),
    DESCBLOQ    VARCHAR(100) NOT NULL
)
...
/* Creando las relacionesentre las tablas (BLOQUE - CONFERENCIA) */
ALTER TABLE CONFERENCIA
    ADD CONSTRAINT BLOQUE_CONFERENCIA FOREIGN KEY (CODBLOQ) REFERENCES BLOQUE(CODBLOQ)
GO

/* Creando las relacionesentre las tablas (BLOQUE - REGISTRODETALLE) */
ALTER TABLE REGISTRODETALLE
    ADD CONSTRAINT BLOQUE_REGISTRODETALLE FOREIGN KEY (CODBLOQ) REFERENCES BLOQUE(CODBLOQ)
GO
...
INSERT INTO BLOQUE
    (NOMBLOQ, DESCBLOQ)
VALUES
    ('Cyberseguridad', 'Conferencia sobre CyberSeguridad'),
    ('Transofrmación Digital', 'Conferencia sobre Transformación Digital'),
    ('Desarrollo Empresarial', 'Conferencia sobre Desarrollo Empresarial')

INSERT INTO CONFERENCIA
    (TEMCONF, CODPON, CODBLOQ)
VALUES
    ('Hackers de sombrero blanco como aliados para resguardar el proceso electoral', 1, 1),
    ('La manipulación en la industria de las redes sociales mediante el uso de bots', 2, 1),
    ('Transformación digital impulsada por los datos', 3, 2),
    ('La TRANSFORMACIÓN DIGITAL en un mundo impulsado por los datos', 4, 2),
    ('SECUENCIA DE ACTIVIDADES DE ADMINISTRACION DE UN PROYECTO', 5, 3),
    ('RECOPILACION DE LOS REQUERIMIENTOS', 6, 3)
GO
...
CREATE VIEW vCONSULTA
AS
SELECT
    R.CODREG,
    p.APEPAR + ', ' + p.NOMPAR as Participante,
    CASE B.NOMBLOQ
        WHEN 'Cyberseguridad' THEN 'Cyberseguridad'
        WHEN 'Transofrmación Digital' THEN 'Transofrmación Digital'
        WHEN 'Desarrollo Empresarial' THEN 'Desarrollo Empresarial'
        END AS BLOQUE,
    C.TEMCONF,
    R.CERTREG,
    P.FECREGPAR,
...
FROM REGISTRO AS R
    ...
    ON RD.CODBLOQ = B.CODBLOQ
    INNER JOIN CONFERENCIA AS C
    ON B.CODBLOQ = C.CODBLOQ
